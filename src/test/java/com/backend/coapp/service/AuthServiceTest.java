...
public class AuthServiceTest {
    ...
    public void setUp(){
        ...
        this.fooUser = new UserModel("foo@mail.com", "password123", "foo", "woof",123);
        this.userRepository.save(fooUser);
    ...
    public void createNewUser_whenEmailAlreadyUsed_expectException(){
        assertThrows(AuthEmailAlreadyUsedException.class,()->this.authService.createNewUser(this.fooUser.getEmail(),"newpassword","foo","woof"));
    }
    ...
    public void createNewUser_whenNewEmailIsUsed_expectEmailSentWith(){
        ...
        verify(this.emailService).sendEmail(eq("newUser@mail.com"),anyString(),emailBodyCaptor.capture());

        String regex = String.format("(?s).*\\b\\d{%d}\\b.*", AuthService.NUMS_VERIFICATION_CODE); // Regex to check if the email body contains 6 digits.
    ...
    public void verifyUser_whenUserNotYetRegistered_expectException(){
        assertThrows(AuthEmailNotRegisteredException.class,()->this.authService.verifyUser("notFoo@mail.com",123));

    }

    @Test
    public void verifyUser_whenUserAlreadyRegisteredWithWrongCode_expectFalseVerificationStatus(){
        boolean verify = this.authService.verifyUser(this.fooUser.getEmail(),000);
        assertFalse(verify);
        assertFalse(this.userRepository.findUserModelByEmail(this.fooUser.getEmail()).getVerified());
    }

    @Test
    public void verifyUser_whenUserAlreadyRegisteredWithCorrectCode_expectTrueVerificationStatus(){
        boolean verify = this.authService.verifyUser(this.fooUser.getEmail(),123);
        assertTrue(verify);
        assertTrue(this.userRepository.findUserModelByEmail(this.fooUser.getEmail()).getVerified());
    }

    @Test
    public void verifyUser_whenUserAlreadyVerified_expectTrueVerificationStatus(){
        this.fooUser.setVerified(true);
    ...
    public void resetVerifyCode_whenUserNotYetRegistered_expectException(){
        assertThrows(AuthEmailNotRegisteredException.class,()->this.authService.resetVerifyCode("notFoo@mail.com"));
    }


    @Test
    public void resetVerifyCode_whenUserAlreadyRegistered_expectEmailSentWithCode(){
        this.authService.resetVerifyCode(this.fooUser.getEmail());
        UserModel currFooUser = this.userRepository.findUserModelByEmail(this.fooUser.getEmail());
        assertNotNull(currFooUser);

        verify(this.emailService, times(1)).sendEmail(eq(this.fooUser.getEmail()),anyString(),anyString());

        ArgumentCaptor<String> emailBodyCaptor = ArgumentCaptor.forClass(String.class);

        verify(this.emailService).sendEmail(eq(this.fooUser.getEmail()),anyString(),emailBodyCaptor.capture());
