...
public class AuthController {
    ...
    public ResponseEntity<Map<String, Object>> signIn(@RequestBody UserRegisterRequest registerRequest) {
        try{
        ...
        } catch (EmailInvalidAddress e){
            //@TODO: add this to API document
            return ResponseEntity.status(400).body(Map.of("error",AuthErrorCodeEnum.INVALID_EMAIL,"message","Provided email is invalid."));
    ...
    public ResponseEntity<Map<String,Object>> verifyEmail (@RequestBody VerifyEmailRequest verifyEmailRequest){
        ...
        try{
            ...
            return ResponseEntity.status(400).body(Map.of("error", RequestErrorCodeEnum.REQUEST_HAS_NULL_OR_EMPTY_FIELD, "message", e.getMessage()));
        }catch (AuthEmailNotRegistered e){
            // @TODO: Added this to API doc
